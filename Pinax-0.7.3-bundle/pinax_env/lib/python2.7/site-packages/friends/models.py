...
class FriendshipManager(models.Manager):
    ...
    def friends_for_user(self, user):
        friends = []
        for friendship in self.filter(from_user=user).order_by('to_user').select_related(depth=1):
            friends.append({"friend": friendship.to_user, "friendship": friendship})
        for friendship in self.filter(to_user=user).order_by('from_user').select_related(depth=1):
...
class Friendship(models.Model):
    ...
    have both agreed to the association.(but need to take care of different friend type from both users)
    """
    FRIEND_TYPE = (
	('acq','Acquaintance'),
	('cf','Close Friend'),
	('pf','Professional Friend'),
	('bf','Blocked Friend'),
	('f','Friend')
    )
    ...
    # @@@ relationship types
    from_type = models.CharField(max_length=5,choices=FRIEND_TYPE)
    to_type = models.CharField(max_length=5,choices=FRIEND_TYPE)
    added = models.DateField(default=datetime.date.today)
...
class JoinInvitation(models.Model):
    ...
    def accept(self, new_user):
        ...
        friendship = Friendship(to_user=new_user, from_user=self.from_user,to_type=self.to_type,from_type=self.from_type)
        friendship.save()
        # notify
        if notification:
...
class FriendshipInvitation(models.Model):
    ...
    def accept(self):
        if not Friendship.objects.are_friends(self.to_user, self.from_user):
            friendship = Friendship(to_user=self.to_user, from_user=self.from_user, to_type=self.to_type,from_type=self.from_type)
	    print("from_type trying ",self.from_type)
            friendship.save()
            self.status = "5"
            self.save()
            if notification:
...
class FriendshipInvitationHistory(models.Model):
    ...
    from_type= models.CharField(max_length=10, choices=FRIEND_TYPE)
    to_type= models.CharField(max_length=10, choices=FRIEND_TYPE)
    message = models.TextField()
    sent = models.DateField(default=datetime.date.today)
    status = models.CharField(max_length=1, choices=INVITE_STATUS)
...
def friendship_invitation(sender, instance, **kwargs):
    ...
    for friendship_invitation in friendship_invitations:
		...
		from_type=friendship_invitation.from_type,
		to_type=friendship_invitation.to_type,
