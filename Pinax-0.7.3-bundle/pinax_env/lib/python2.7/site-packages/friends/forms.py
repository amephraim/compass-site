...
class InviteFriendForm(UserForm):
    ...
    from_type = forms.ChoiceField(label="from_type",choices=FRIEND_TYPE)
    ...
    def save(self):
        to_user = User.objects.get(username=self.cleaned_data["to_user"])
	from_type = self.cleaned_data["from_type"]
        message = self.cleaned_data["message"]
        invitation = FriendshipInvitation(from_user=self.user, to_user=to_user,from_type=from_type, message=message, status="2")
        invitation.save()
        if notification:
