...
from compass_tweets.models import Context,ContextMember,ContextInstance
from tribes import models as tribemodels
...
context_re = re.compile(r'(?<=\W)[#]\S*')
message_re = re.compile("^!(\w+)")
...
class Tweet(models.Model):
    ...
    #contextid = models.ManyToManyField(ContextInstance)    
...
class TweetInstance(models.Model):
    ...
    tribe = models.ManyToManyField(tribemodels.Tribe)
    ...
    def get_context(self):
        return "\n".join([s.name for s in self.contextid.all()])
def tweet(sender, instance, created, **kwargs):
    ...
    # add sender
    recipients.add(user)
    print "checking cntext present?"    
    print re.findall(r'(?<=\W)[#]\S*', instance.text)
    match1 = context_re.findall(instance.text)
    if match1:
	print "context given"
        contextid = match1[0][1:]
	print contextid
    # if starts with @user send it to them too even if not following
    contextmembers = ContextMember.objects.filter(context = "MyClass")
    print "members",contextmembers
	...
	tweet_instance.contextid.add(contextid)
